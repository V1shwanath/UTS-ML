...
class FunctionInfo:
	...
	def __init__(self):
		...
		self.func_dict =   {	
								'TKPY_Kmeans_Clust_Centers': [['float'], ['matrix'], ['cluster centers']],
                                'TKPY_Kmeans_predVal' : [['float','float'],['float'],['Predict the cluster for a datapoint']],
								'TKPY_Kmeans_DataCluster': [['float'],['array'],['clusters in the dataset']],
								"TKPY_Kmeans_elbow_plot" : [['float'],['array'],['elbow plot for clustering']]
...
def TKPY_Kmeans_DataCluster(a):
	clusters = clust + a -a

	return  np.array(clusters)

def TKPY_Kmeans_predVal(a,b):
	pred= km.predict([[a,b]])
	return(float(pred))

def TKPY_Kmeans_Clust_Centers(a):
	points = cluster_centers + a-a
	return np.array(points)

def TKPY_Kmeans_elbow_plot(a):
	sse = []
